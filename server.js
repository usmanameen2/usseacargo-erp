...
authRouter.post('/login', async (req, res) => {
  try {
    ...
    if (!userCount || userCount.c === 0) {
      ...
      adminExpiry.setFullYear(adminExpiry.getFullYear() + 10); // Admin never expires
      const result = await dbRun(
        ...
        ['admin', hash, 'admin@usseacargo.com', 'System Administrator', 'admin', 'USSeaCargo Inc.', '+971-4-123-4567', 'AD', 1, '1_year', adminExpiry.toISOString(), 'active']
      );
      await dbRun(
  ...
  try {
    const companies = await dbAll(`
      SELECT c.*, u.username as owner_username, u.full_name as owner_name, u.email as owner_email,
             u.phone as owner_phone, u.subscription_plan, u.subscription_expiry, u.subscription_status
      FROM companies c
