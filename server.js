...
authRouter.post('/register', async (req, res) => {
  try {
    ...
    const result = await dbRun('INSERT INTO users (username, password_hash, email, full_name, role, company_name, avatar_initials, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, 1)', [username, hash, email, full_name, 'user', company_name || 'USSeaCargo', initials]);
    const token = jwt.sign({ userId: result.lastID, username, role: 'user' }, JWT_SECRET, { expiresIn: '7d' });

    // Track company signup
    try {
      await dbRun('INSERT INTO companies (company_name, contact_email, contact_person, created_by_user_id) VALUES (?, ?, ?, ?)',
        [company_name || 'USSeaCargo', email, full_name, result.lastID]);
