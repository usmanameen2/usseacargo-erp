...
async function init() {
  ...
  // Auto-seed admin user if no users exist (for fresh deployments)
  const userCheck = database.prepare("SELECT COUNT(*) as c FROM users");
  userCheck.step();
  const userCount = userCheck.getAsObject().c || 0;
  userCheck.free();
  if (userCount === 0) {
    ...
    adminStmt.bind(['admin', adminHash, 'admin@usseacargo.com', 'Administrator', 'admin', 'USSeaCargo Inc.', '+1-555-0100', 'AD']);
    adminStmt.step();
    adminStmt.free();
    persist();
