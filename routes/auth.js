...
router.post('/login', (req, res) => {
  try {
    ...
    // AUTO-SEED: If no users exist, create admin on first login attempt
    const userCount = db.prepare('SELECT COUNT(*) as c FROM users').get();
    if (userCount.c === 0 && username === 'admin' && password === 'admin123') {
      console.log('[Auth] Auto-creating admin user on first login...');
      const passwordHash = bcrypt.hashSync('admin123', 10);
      const result = db.prepare(
        `INSERT INTO users (username, password_hash, email, full_name, role, company_name, phone, avatar_initials, is_active)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).run('admin', passwordHash, 'admin@usseacargo.com', 'Administrator', 'admin', 'USSeaCargo Inc.', '+1-555-0100', 'AD', 1);

      // Create default company settings
      db.prepare(
        `INSERT INTO company_settings (user_id, company_name, base_currency, decimal_places)
         VALUES (?, ?, ?, ?)`
      ).run(result.lastInsertRowid, 'USSeaCargo Inc.', 'USD', 2);

      const token = generateToken(result.lastInsertRowid, 'admin', 'admin');
      return res.json({
        success: true,
        ...
        data: {
          ...
          user: {
            ...
            username: 'admin',
            email: 'admin@usseacargo.com',
            full_name: 'Administrator',
            role: 'admin',
            company_name: 'USSeaCargo Inc.',
            avatar_initials: 'AD'
          }
        }
      });
